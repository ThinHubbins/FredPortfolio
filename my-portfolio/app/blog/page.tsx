...
export default function Home() {
  const [status, setStatus] = useState<"idle" | "loading" | "success" | "error" | "duplicate">("idle") // 👈 "idle" not "duplicate"
  const [email, setEmail] = useState("")

  const skills = ["React", "Node.js", "MongoDB", "Express.js", "Bootstrap", "Next.js", "Tailwind CSS", "GSAP"];
  const marquee = ["Frontend Dev", "Backend Architect", "API Design", "Database Modeling", "Open Source", "Technical Writing", "CI/CD Pipelines", "System Design"];
  ...
  async function subscribe(e: React.FormEvent) {
    ...
    if (error) {
      if (error.code === "23505") {
        setStatus("duplicate") // 👈 fixed spelling
      } else {
        console.log(error)
        setStatus("error")
      }
  ...
  return (
    <main className="flex min-h-screen flex-col bg-gray-100 dark:bg-black text-black dark:text-white">
      ...
      <div className="p-4 sm:p-6">
        ...
        <div className="flex flex-col sm:flex-row w-full items-start sm:items-end justify-between gap-10 sm:gap-6">
          <div className="flex-1">
            ...
            <div className="mt-10 sm:mt-30 flex flex-wrap max-w-md items-start gap-3">
              {skills.map((skill) => (
                <div key={skill} className="py-1 px-3 border border-gray-400 text-sm font-light">
                  {skill}
                </div>
              ))}
            </div>
        ...
        <Marquee speed={100} gradient={false} className="bg-black dark:bg-gray-200 text-white dark:text-black font-normal mt-16 sm:mt-20">
          {marquee.map((item) => (
            <div key={item} className="py-3 px-2">{item} •</div>
          ))}
        </Marquee>
            ...
            <div className="relative z-10 flex flex-col sm:flex-row items-center gap-3 w-full max-w-md">
              <form onSubmit={subscribe} className="w-full flex flex-col sm:flex-row gap-3">
                ...
                <button
                  ...
                  className="w-full sm:w-auto whitespace-nowrap px-6 py-3 bg-gray-900 dark:bg-white text-white dark:text-gray-900 font-semibold rounded-xl hover:bg-gray-700 dark:hover:bg-gray-500 transition-all duration-300 text-sm disabled:opacity-50"
                >
